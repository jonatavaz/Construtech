class AppUrl {
  static const String baseUrl = 'https://construtech.igprojetohub.targetbr.biz/'; 

  static const String construtechApiPath = 'construtechApi';

  static const int httpRequestTimeout = 30;

  static const String genericErrorMessage = 'Ocorreu um erro. Tente novamente.';
  static const String networkErrorMessage = 'Problema de conexão. Verifique sua internet.';
}